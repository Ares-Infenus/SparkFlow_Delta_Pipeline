...
def add_time_features(df: DataFrame) -> DataFrame:
    ...
    return (
        ...
        .withColumn("is_night", F.when((F.col("hour") >= 22) | (F.col("hour") <= 5), 1).otherwise(0))
    )
...
def add_rolling_account_stats(df: DataFrame, window_days: int = 7) -> DataFrame:
    ...
    return (
        df
        .withColumn("rolling_avg_amount", F.round(F.avg("amount").over(window_spec), 2))
...
def add_amount_zscore(df: DataFrame) -> DataFrame:
    ...
    return (
        df
        .join(account_stats, on="account_id", how="left")
        .withColumn(
            "amount_zscore",
            F.when(
                F.col("account_std_amount") > 0,
                F.round((F.col("amount") - F.col("account_avg_amount")) / F.col("account_std_amount"), 4),
            ).otherwise(F.lit(0.0)),
        )
